...
<div class="container mt-4 mb-5">
    <div class="payment-card">
        ...
        {{-- DETAIL PESANAN --}}
        <div class="mt-3">
            <div class="label-box"><strong>Kode Order:</strong> {{ $pesanan->kode_order }}</div>
            <div class="label-box"><strong>Total Bayar:</strong> <span class="text-danger fw-bold">Rp {{ number_format($pesanan->total_bayar,0,',','.') }}</span></div>
            <div class="label-box"><strong>Metode Pengiriman:</strong> {{ $pesanan->metode_pengiriman }}</div>
            <div class="label-box">
            ...
            @if($pesanan->alamat)
                <div class="label-box">
                    ...
                    Kode Pos: {{ $pesanan->alamat->kode_pos ?? '-' }}
                </div>
            @endif
        </div>
        ...
        {{-- UPLOAD BUKTI --}}
        @if(!$pesanan->buktiPembayaran)
            <form action="{{ route('payment.upload', $pesanan->id) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                @csrf
                <div class="upload-box text-center">
                    <p class="mt-2">Upload Bukti Pembayaran</p>
                    <input type="file" name="bukti_pembayaran" id="foto_bukti" class="form-control mt-2" required onchange="enablePayButton()">
                </div>

                <div class="d-flex gap-2 mt-3">
                    <a href="{{ route('payment.index') }}" class="btn btn-secondary w-50">Kembali</a>
                    <button type="submit" id="btnSudahBayar" class="btn-pay w-50" disabled>Sudah Bayar</button>
                </div>
            </form>
        @else
            ...
            <img src="{{ asset('uploads/bukti/' . $pesanan->buktiPembayaran->bukti_pembayaran) }}" class="img-fluid mt-3 rounded shadow" width="320">
        @endif
